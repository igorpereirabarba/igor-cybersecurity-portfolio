...
// Cursor + magnetic interaction — restored to the original site behavior.
// Use any-pointer/any-hover so a desktop mouse still works on touch-capable PCs.
if (finePointer) {
  // The original stylesheet hides the custom cursor for coarse/touch pointers.
  // On hybrid Windows systems both coarse and fine pointers may exist, so force
  // the cursor back on whenever a real fine/hover-capable pointer is available.
  const cursorOverride = document.createElement('style');
  cursorOverride.textContent = '.cursor{display:block !important;}';
...
if (structuredProfile) {
  try {
    ...
    data.sameAs = ['https://github.com/igorpereirabarba','https://www.linkedin.com/in/igor-barbosa-12a072280'];
...
if (projectsSection && !document.getElementById('home-lab-experience')) {
  ...
  labSection.innerHTML = `
    <div class="shell">
      ...
      <div class="home-lab-grid">
        <a class="lab-experience-card spotlight-card magnetic" href="https://github.com/igorpereirabarba/home-soc-lab" target="_blank" rel="noopener"><small>01 / SECURITY OPERATIONS</small><h3>SOC Monitoring & Detection</h3><p>Centralized Windows and Sysmon telemetry in Splunk, investigated failed-authentication activity, built SPL searches and mapped behavior to MITRE ATT&CK.</p><span>Splunk · Sysmon · Event 4625 · T1110 ↗</span></a>
        <a class="lab-experience-card spotlight-card magnetic" href="https://github.com/igorpereirabarba/linux-server-administration" target="_blank" rel="noopener"><small>02 / INFRASTRUCTURE</small><h3>Linux Server Administration</h3><p>Operate an Ubuntu server, configure SSH, manage users/groups and permissions, validate services, networking and controlled shared resources.</p><span>Ubuntu · SSH · systemctl · permissions ↗</span></a>
        <a class="lab-experience-card spotlight-card magnetic" href="https://github.com/igorpereirabarba/network-security-windows-hardening" target="_blank" rel="noopener"><small>03 / ENDPOINT + NETWORK</small><h3>Hardening & Traffic Analysis</h3><p>Practice packet analysis, host discovery, Windows security controls, firewall review, logging, patching and endpoint visibility.</p><span>Wireshark · Nmap · Windows · Sysmon ↗</span></a>
        <a class="lab-experience-card spotlight-card magnetic" href="https://github.com/igorpereirabarba/security-log-automation" target="_blank" rel="noopener"><small>04 / AUTOMATION</small><h3>Security Automation</h3><p>Use Python to process failed-login records, apply detection thresholds and generate structured analyst alerts and incident reports.</p><span>Python · detection logic · reporting ↗</span></a>
      </div>
    </div>`;
  ...
  // Bind the injected cards to the exact same original cursor choreography.
  if (finePointer) {
    labSection.querySelectorAll('a, button, .lab-experience-card').forEach(el => {
      el.addEventListener('mouseenter', () => document.body.classList.add('pointer-hover'));
      el.addEventListener('mouseleave', () => document.body.classList.remove('pointer-hover'));
    });
    labSection.querySelectorAll('.magnetic').forEach(el => {
      el.addEventListener('mousemove', e => {
        const r = el.getBoundingClientRect();
        const x = e.clientX - r.left - r.width / 2;
        const y = e.clientY - r.top - r.height / 2;
    ...
    labSection.querySelectorAll('.spotlight-card').forEach(el => {
      el.addEventListener('mousemove', e => {
        const r = el.getBoundingClientRect();
        el.style.setProperty('--mx', `${e.clientX - r.left}px`);
        el.style.setProperty('--my', `${e.clientY - r.top}px`);
